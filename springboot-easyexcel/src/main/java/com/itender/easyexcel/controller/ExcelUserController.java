...
public class ExcelUserController {
    ...
    @Autowired
    public ExcelUserController(UserService userService) {
        this.userService = userService;
    }

    @GetMapping("/export/unannotated")
    public void exportUserByUnAnnotation(HttpServletResponse response) {
        try {
            response.reset();
            ...
            userService.exportUserByUnAnnotation(response.getOutputStream());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
